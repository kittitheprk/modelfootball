...
def main():
    if len(sys.argv) < 3:
        print("Usage: python ai_match_analyst.py <HomeTeam> <AwayTeam>")
        return
    ...
    # 2. Identify League -> Assuming both teams same league for simplicity here
    # Find home team league
    home_players = player_df[player_df['Squad'].str.contains(home_name, case=False)]
    if home_players.empty:
        print(f"Team {home_name} not found in player DB.")
        return
    league = home_players.iloc[0]['League']
    ...
    # 6. Generate Pizza Charts for Key Players (Top SCAs)
    # Pick Top 1 player from each team to show example
    ...
    key_away = player_df[player_df['Squad'].str.contains(away_name, case=False)].sort_values(by='Shot_Creating_Actions', ascending=False).iloc[0]
